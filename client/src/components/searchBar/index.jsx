...
const SearchBar = () => {
  ...
  useEffect(() => {
    if(isClickedOutside){ // if it's true...
        collapseContainer()
    }
  }, [isClickedOutside]);
  ...
  const prepareSearchQuery = (query) => {
    const api = `https://api.rawg.io/api/games?key=d063d7ea72004c9a8f9461144dd91d96&search=${query}&page_size=10`;
  ...
  const searchGame = async () => {
    ...
    const response = await axios.get(URL).catch((err) => console.log('Error Api Data: ', err))
